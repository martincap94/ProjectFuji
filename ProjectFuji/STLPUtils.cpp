...
float computePseudoadiabaticLapseRate(float T, float P) {
	...
	float Lv = computeLatentHeatOfVaporisation(T);

	float res = (1.0f + w) * (1.0f + (Lv * w) / (R_d * T));
	res /= c_pd + w * c_pv + (Lv * Lv * w * (EPS + w)) / (R_d * T * T);
	res *= (-9.81f);
...
float computeEquivalentTheta(float T, float dewPoint, float P) {
	...
	float w = getMixingRatioOfWaterVapor(dewPoint, P);
...
float e_s(float T) {
	return 611.2f * exp((17.67f * (T - 273.15f)) / (T - 29.65f));
}
...
float dTdp_moist(float T, float p) {
	return dTdz_moist(T, p) * -(R_d * T) / (p * 9.81f);
}
